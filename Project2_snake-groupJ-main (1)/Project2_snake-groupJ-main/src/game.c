...
void UpdateGame(GameData *game, Snake *snake, Food *food) {
  ...
  if (game->state == STATE_START) {
    ...
    if (game->isPaused) {
      ...
      // Resume Button logic (must match DrawGame)
      Rectangle resumeRect = {WINDOW_WIDTH / 2.0f - 150, WINDOW_HEIGHT / 2.0f - 60, 300, 80};
      if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && CheckCollisionPointRec(mPos, resumeRect)) {
        game->isPaused = false;
      }

      // Back to Menu Button logic (must match DrawGame)
      Rectangle backRect = {WINDOW_WIDTH / 2.0f - 225, WINDOW_HEIGHT / 2.0f + 40, 450, 80};
      if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && CheckCollisionPointRec(mPos, backRect)) {
        game->isPaused = false;
        game->state = STATE_START;
      }
    }
